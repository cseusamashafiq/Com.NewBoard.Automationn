...
public class TC_166 extends LambdaTestDriver{
	...
	@Test(groups = {"Add Link in Inbox", "CompleteRegressionSuite"})
	public void newBoardLogin() throws Exception {
		...
		ltOptions.put("build", "TC_166_Inbox");
		ltOptions.put("project", "TC_166_Inbox");
		ltOptions.put("name", "TC_166_Inbox");
		...
		Thread.sleep(5000);
		try {
			WebElement inboxTab = driver.findElement(By.xpath("//a[contains(@href, '/inbox')]")); // Replace with actual locator
			inboxTab.click();

			// Wait for the Inbox page to load
			Thread.sleep(3000);

			// Click on "Add Link" button //button//p[contains(text(), 'Add Link')]
			WebElement addLinkButton = driver.findElement(By.xpath("//button//p[contains(text(), 'Add Link')]")); // Replace with actual locator
			addLinkButton.click();
